...
from trans.module.model.TransE_AddTime import  TransE_AddTime
...
parser = argparse.ArgumentParser(description='Ttrane_tp_time_predict')
...
parser.add_argument('-dataset_path',dest = 'dataset_path', default = './benchmarks/data/yago_data_addTime_year_tp2id_filter/',help = 'dataset_path')
...
args.checkpoint = "./checkpoint_"+args.data_process + "/Ttranse_time_predict/"+args.data_type+"/"+str(args.dim)+"_"+str(args.margin)+"_"+str(args.lr)+"/train"+".ckpt"
...
train_tester = Tester_AddTime(model = TtransE, data_loader = valid_dataloader_for_test, use_gpu = True,constraint_file = args.dataset_path+"r_time_static.txt")
...
# trainer.run()
TtransE.save_checkpoint(str(args.checkpoint))
...
test_dataloader = PyTorchTestDataLoader(
	...
	ent_tot = TtransE.ent_tot,
	rel_tot = TtransE.rel_tot,
 	tp_tot = train_dataloader.get_tp_tot()
 )
print("test……")
# test the model
TtransE.load_checkpoint(str(args.checkpoint))
tester = Tester_AddTime(model = TtransE, data_loader = test_dataloader, use_gpu = True, constraint_file = args.dataset_path+"r_time_static.txt")
