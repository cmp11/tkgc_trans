...
class PyTorchTrainDataset(Dataset):
	...
	def __init__(self, head, tail, rel, tp_point, 
              ent_total, rel_total, sampling_mode = 'normal', bern_flag = False, filter_flag = True, neg_ent = 1, neg_rel = 0,
              neg_time=0):
		# triples
		self.head = head
		self.tail = tail
...
class PyTorchTrainDataLoader(DataLoader):
	...
	def __init__(self, 
		...
		dataset = self.__construct_dataset(sampling_mode, bern_flag, filter_flag, neg_ent, neg_rel, time_point_formate)
	...
	def __construct_dataset(self, sampling_mode, bern_flag, filter_flag, neg_ent, neg_rel, time_point_formate):
		...
		dataset = PyTorchTrainDataset(np.array(head_tp), np.array(tail_tp), np.array(rel_tp), np.array(tp_point), 
                                ent_total, rel_total, sampling_mode, bern_flag, filter_flag, neg_ent, neg_rel)
