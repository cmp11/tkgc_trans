...
class TrainDataLoader(object):
	...
	def __init__(self, 
        # 各参数的含义可参考以下文档：
        # https://pybind11-openke.readthedocs.io/zh-cn/stable/api/generated/pybind11_ke.data.TrainDataLoader.html     
		in_path = "./", # 数据集目录
		tri_file = None, #  train2id.txt
		ent_file = None, # entity2id.txt
		rel_file = None, # relation2id.txt
		batch_size = None,  # batch_size 可以根据 nbatches 计算得出，两者不可以同时不提供；同时指定时 batch_size 优先级更高
		nbatches = None,
		threads = 8,
		sampling_mode = "normal", # 数据采样模式，normal 表示正常负采样，cross 表示交替替换 head 和 tail 进行负采样
		bern_flag = False,  # 是否使用 TransH 提出的负采样方法进行负采样
		filter_flag = True,
		neg_ent = 1, # 对于每一个正三元组, 构建的负三元组的个数, 替换 entity (head + tail)
		neg_rel = 0): # 对于每一个正三元组, 构建的负三元组的个数, 替换 relation
