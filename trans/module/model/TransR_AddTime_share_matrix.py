...
'''
transR的算法模型，将实体和关系映射到关系的空间维度
映射时，使用共享矩阵 即所有的实体用同一个矩阵进行映射，所有的关系用同一个矩阵映射
'''
class TransR_AddTime(Model):
	...
	def __init__(self, ent_tot, rel_tot, tp_tot,dim_e = 100, dim_r = 100, dim_tp = 100,  p_norm = 1, norm_flag = True, rand_init = False, margin = None):
		super(TransR_AddTime, self).__init__(ent_tot, rel_tot,tp_tot)
		...
		self.e_to_r_transfer_matrix = nn.Embedding(self.dim_e , self.dim_r)
		self.tp_to_r_transfer_matrix = nn.Embedding( self.dim_tp , self.dim_r)
  
		if not self.rand_init:
			identity = torch.zeros(self.dim_e, self.dim_r)
			for i in range(min(self.dim_e, self.dim_r)):
				identity[i][i] = 1
			self.e_to_r_transfer_matrix.weight.data.copy_(identity)

			identity = torch.zeros(self.dim_tp, self.dim_r)
			for i in range(min(self.dim_tp, self.dim_r)):
				identity[i][i] = 1
			self.tp_to_r_transfer_matrix.weight.data.copy_(identity)
	...
	def _e_transfer(self, e):
		...
		transfer_matrix = self.e_to_r_transfer_matrix.weight  # [dim_e, dim_tp]
		return torch.matmul(e, transfer_matrix)   # [batch_size, dim_tp]
    	

	def _tp_transfer(self, tp):
		transfer_matrix = self.tp_to_r_transfer_matrix.weight  # [dim_r, dim_tp]
		return torch.matmul(tp, transfer_matrix)   # [batch_size, dim_tp]
	...
	def regularization(self, data):
		...
		e_to_r_transfer = self.e_to_tp_transfer_matrix.weight
		tp_to_r_transfer = self.r_to_tp_transfer_matrix.weight
