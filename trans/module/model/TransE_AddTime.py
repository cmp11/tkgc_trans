...
class TransE_AddTime(Model):
	...
	def __init__(self, ent_tot, rel_tot,tp_tot, dim = 100, p_norm = 1, norm_flag = True, margin = None, epsilon = None):
		...
		self.tp_embeddings = nn.Embedding(self.tp_tot, self.dim)
		...
		if margin == None or epsilon == None:
			...
			nn.init.uniform_(
				tensor = self.tp_embeddings.weight.data, 
				a= -self.embedding_range.item(), 
				b= self.embedding_range.item()
			)
