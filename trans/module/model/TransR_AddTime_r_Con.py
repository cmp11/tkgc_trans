...
'''
class TransR_AddTime_r_Con(Model):
    ...
    def __init__(self, ent_tot, rel_tot, tp_tot, tp_min, dim_e = 100, dim_r = 100, dim_tp = 100,  p_norm = 1, norm_flag = True, rand_init = False, margin = None):
        super(TransR_AddTime_r_Con, self).__init__(ent_tot, rel_tot,tp_tot)
    ...
    def cosine_similarity_matrix(self, A, B):
        ...
        cos_sim_score = similarity_matrix.flatten()
    ...
    def forward(self, data):
        ...
        if not data['is_test']:
            batch_h_con = data['batch_h_con']
            batch_t_con = data['batch_t_con']
            batch_r_con = data['batch_r_con']
            batch_tp_con = data['batch_tp_con']
            
            h_con = self.ent_embeddings(batch_h_con)
            t_con = self.ent_embeddings(batch_t_con)
            r_con = self.rel_embeddings(batch_r_con)
            tp_con = self.tp_embeddings(batch_tp_con)
            e_to_r_transfer_con = self.e_to_r_transfer_matrix(batch_r_con)
            tp_to_r_transfer_con = self.tp_to_r_transfer_matrix(batch_r_con)
            h_con = self._e_transfer(h_con, e_to_r_transfer_con)
            t_con = self._e_transfer(t, e_to_r_transfer_con)
            tp_con = self._tp_transfer(tp, tp_to_r_transfer_con)
            score = self._calc(h, t, r, tp, mode)
            
            r_tp_cos_score = self.cosine_similarity_matrix(r_con, tp_con)
